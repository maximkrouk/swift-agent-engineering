...
final class FeatureModel {
  ...
  var clock  // Controllable way to sleep a task
  @ObservationIgnored
  @Dependency(\.date.now)
  var now  // Controllable way to ask for current date
  @ObservationIgnored
  @Dependency(\.mainQueue)
  var mainQueue  // Controllable scheduling on main queue
  @ObservationIgnored
  @Dependency(\.uuid)
  var uuid  // Controllable UUID creation
...
inside the `addButtonTapped` method, you can use the [withDependencies](../extensions/with-dependencies.md)
  ...
  #expect(
    model.items == [
      Item(
        ...
        name: "",
        createdAt: .init(timeIntervalSinceReferenceDate: 1234567890)
      )
    ]
  )
}
...
can do. You can learn more in depth about [WhatAreDependencies](what-are-dependencies.md) as well as
[UsingDependencies](using-dependencies.md). Once comfortable with that you can learn about
[RegisteringDependencies](registering-dependencies.md) as well as how to best leverage [LivePreviewTest](live-preview-test.md). And finally,
there are more advanced topics to explore, such as [DesigningDependencies](designing-dependencies.md),
[OverridingDependencies](overriding-dependencies.md), [Lifetimes](lifetimes.md) and [SingleEntryPointSystems](single-entry-point-systems.md).
